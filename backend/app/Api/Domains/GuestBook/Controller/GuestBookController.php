...
class GuestBookController extends ResourceController
{
    ...
    public function showLimit($limit = null)
    {
        ...
        $q = $gBook->orderBy('created_at', 'DESC')->limit($limit)->offset(0)->
            select("guest_books.id id, guest_books.comment comment, u.id u_id, u.name u_name, u.email u_email, u.image_url u_img_url, u.bio u_bio, b.badge_type, guest_books.created_at, guest_books.updated_at")->
            join('users u', 'u.id = guest_books.user_id')->join('badge_inventories b', 'b.user_id = guest_books.user_id');
        $r = $q->get()->getResult();
